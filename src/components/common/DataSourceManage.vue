            ...
            <el-form :model="formData" label-width="70px">
                ...
                <el-form-item label="驱动类型">
                    <el-select v-model="formData.driverClass" filterable placeholder="请选择">
                        <el-option v-for="item in items" :key="item.index" :label="item.name" :value="item.driverClass">
                        </el-option>
                    </el-select>
                </el-form-item>
            </el-form>
    ...
    export default {
        data() {
            return {
                ...
                flag: true,
                items: [
                    {name:'mysql',driverClass:'com.mysql.jdbc.Driver'},
                    {name:'oracle',driverClass:'oracle.jdbc.OracleDriver'},
                    {name:'presto',driverClass:'com.facebook.presto.jdbc.PrestoDriver'},
                    {name:'kylin',driverClass:'org.apache.kylin.jdbc.Driver'}
                ],
            };
        },
        created() {
        ...
        computed: {
            ...
            data() {
                return this.tableData.filter(data => {
                    if (data.name.indexOf(this.select_word) > -1 || data.url.indexOf(this.select_word) > -1) {
                        return data;
                    }
                });
            }
        ...
        methods: {
            ...
            saveForm() { // 保存
                ...
                this.$requestData('/dataSource/save', 'post', param).then(res => {
            ...
            deleteRow(rowId) {
                ...
                this.$confirm("删除后将不可恢复，是否确认？", "提示", {
                    ...
                    .then(() => {
                        let url = "/dataSource/delete/" + rowId;
